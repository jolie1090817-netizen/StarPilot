...
def batch_scrape_urls(urls_file, output_file=None, delay=2):
    ...
    for idx, url in enumerate(urls, 1):
        ...
        if result['status'] == 'success':
            print(f"  ✅ 標題: {result['title'][:50]}{'...' if len(result['title']) > 50 else ''}")
    ...
    print(f"成功: {sum(1 for r in results if r['status'] == 'success')}/{len(urls)}")
    print(f"失敗: {sum(1 for r in results if r['status'] != 'success')}/{len(urls)}")
    print("-" * 80)
    ...
    try:
        with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
            fieldnames = ['url', 'title', 'status', 'error_message', 'timestamp']
