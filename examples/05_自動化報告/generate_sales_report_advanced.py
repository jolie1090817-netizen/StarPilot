...
# ============ 設定 ============
DATA_FILE = "../../data/sales.csv"  # 主要資料檔案
OUTPUT_DIR = "reports"             # 報告輸出目錄
ARCHIVE_DIR = "reports/archive"    # 過檔報告位置
BACKUP_COUNT = 10                  # 保留最近 N 份報告
...
print(f"   - 週環比: {week_on_week:+.1f}%")

# ============ 建立 Excel 報告 ============
print("\n📝 建立 Excel 報告...")

output_file = f"{OUTPUT_DIR}/週報_{datetime.now().strftime('%Y-%m-%d')}.xlsx"
...
if len(recent_reports) > BACKUP_COUNT:
    for old_report in recent_reports[BACKUP_COUNT:]:
        src = os.path.join(OUTPUT_DIR, old_report)
        dst = os.path.join(ARCHIVE_DIR, old_report)
        shutil.move(src, dst)
        print(f"   ✓ 已備份: {old_report}")
...
- 總銷售額: NT${df['金額'].sum():,}
- 總銷售量: {df['數量'].sum()} 件
- 平均訂單額: NT${df['金額'].mean():.0f}
- 週環比: {week_on_week:+.1f}%
- 銷售商品數: {len(product_stats)} 個
...
print(f"📊 包含工作表: 原始數據、統計摘要、產品排行、銷售趨勢")
print(f"📈 包含圖表: 產品排行、銷售佔比、銷售趨勢")
print(f"\n🎉 每週自動執行，無需手動操作！")
